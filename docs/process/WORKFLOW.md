...
- **Dispatch sizing**: each agent has a hard `maxTurns` cap set in `.claude/agents/*.md` frontmatter (the agent files are the source of truth for the value). Formal file caps are ≤ 15 per dispatch (uniform changes) and ≤ 10 (non-uniform). Wholesale edits across many page files must be split into Same-type parallel batches — see DELEGATION.md → Dispatch Sizing for the contract, the `fork-join-delegation` skill for empirical calibration (`≤ 8` when tests are included, three-phase turn budget pattern), and `docs/reference/DELEGATION_DESIGN_NOTES.md` §1–§2 for incident history.
