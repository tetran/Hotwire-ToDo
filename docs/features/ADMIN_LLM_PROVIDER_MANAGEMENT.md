...
| `app/javascript/admin/pages/llm-providers/LlmProviderWorkspacePage.tsx` | プロバイダー Workspace（Info + Models） |
...
> `llm_models_count` は一覧APIレスポンスでのみ返されます（各 provider に紐づく `LlmModel` 数）。
...
> モデル一覧は provider-scoped かつ件数が bounded のため **pagination を行わず全件返却** する（レスポンスに `meta` は含まれない）。プロバイダー一覧はdropdown cap + Sentry truncation warningで扱う (別仕様)。
...
| `active` | boolean | 有効/無効 |
| `default_model` | boolean | デフォルトモデル指定 |
| `name` | - | update時は無視される（FE は送信しない / BE strong paramsで除外、多層防御） |
