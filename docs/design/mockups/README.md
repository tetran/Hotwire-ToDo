...
- **Ownership**: this is the **`ui-designer` subagent's canonical output directory**. The `.claude/hooks/pre_tool_use_denylist.sh` hook contains a narrow allowlist exception so that `ui-designer` (and only this agent) can Write/Edit under `docs/design/mockups/**`; all other `docs/**` paths remain orchestrator-owned. The orchestrator commits the approved HTML to git, creates the gist, and posts the URL as part of the Post-approval contract (see `.claude/agents/ui-designer.md` § Post-approval responsibilities).
