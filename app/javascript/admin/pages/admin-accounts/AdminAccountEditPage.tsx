...
export const AdminAccountEditPage = () => {
  ...
  useEffect(() => {
    if (!id) return
    adminAccountsApi.get(Number(id))
      .then(account => {
        setEmail(account.email)
        setName(account.name)
      })
      .catch(err => setError(err instanceof Error ? err.message : 'Failed to load admin account'))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      await adminAccountsApi.update(Number(id), { email, name })
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-800"
              style={{ fontFamily: 'Syne, sans-serif' }}>Edit Admin Account</h1>
        </div>
      </div>
