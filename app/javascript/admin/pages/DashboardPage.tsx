...
export const DashboardPage = () => {
  ...
  useEffect(() => {
    setLoading(true)
    ;(async () => {
      const [resDash, resProv] = await Promise.allSettled([
        dashboardApi.get(),
      ...
      if (resDash.status === 'fulfilled') {
        ...
        setDashboardError(buildSectionErrorMessage(resDash.reason))
      }

      if (resProv.status === 'fulfilled') {
        setProviders(resProv.value.llm_providers)
        setProvidersError('')
        reportTruncation({
          resource: 'llm_providers',
          fetched: resProv.value.llm_providers.length,
          total_count: resProv.value.meta?.total_count,
          per_page: DROPDOWN_PER_PAGE,
        })
      } else {
        setProvidersError(buildSectionErrorMessage(resProv.reason))
      }
